...
export function HeavyComputation() {
  const [computationState, setComputationState] = useState<"idle" | "preparing" | "computing" | "complete">("idle")
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2">
                {computationState === "idle" && <span className="text-sm font-medium">Ready to compute</span>}
                {computationState === "preparing" && (
                  <div className="flex items-center space-x-2">
                    <Loader2 className="h-4 w-4 text-blue-500 animate-spin" />
                    <span className="text-sm font-medium">Preparing computation...</span>
                  </div>
                )}
                {computationState === "computing" && (
                  <div className="flex items-center space-x-2">
                    <Loader2 className="h-4 w-4 text-yellow-500 animate-spin" />
                    <span className="text-sm font-medium">Computing prime numbers...</span>
                  </div>
                )}
                {computationState === "complete" && (
                  <div className="flex items-center space-x-2">
                    <CheckCircle className="h-4 w-4 text-green-500" />
                    <span className="text-sm font-medium">Computation complete!</span>
                  </div>
                )}
              </div>

              {(computationState === "computing" || computationState === "complete") && (
                <div className="flex items-center space-x-1">
                  <Clock className="h-3.5 w-3.5 text-zinc-500" />
                  <span className="text-xs font-mono">{elapsedTime.toFixed(2)}s</span>
                </div>
              )}
            </div>
            ...
            {(computationState === "computing" || computationState === "complete") && (
              <div className="space-y-1">
                <div className="relative h-2 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                  <motion.div
                    className={`absolute top-0 left-0 h-full ${computationState === "complete" ? "bg-green-500" : "bg-yellow-500"}`}
                    initial={{ width: 0 }}
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="text-xs text-zinc-500 mt-2">
                  Notice how the main thread stays responsive while the worker thread handles the heavy computation.
                </div>
              </div>
            </motion.div>
          )}
          ...
          {result && (
            ...
            >
              <div className="flex items-center space-x-2 mb-2">
                <CheckCircle className="h-4 w-4 text-green-500" />
                <span className="text-sm font-medium text-green-800 dark:text-green-200">Computation Results:</span>
              </div>
              <div className="space-y-2">
              ...
              <div className="mt-2 text-xs text-green-700 dark:text-green-300">
                By using a Promise or Web Worker, we can run CPU-intensive tasks without blocking the main thread,
