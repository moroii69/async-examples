...
export function AsyncLoop() {
  ...
  const handleRun = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      setItems((prev) => prev.map((item, idx) => (idx === i ? { ...item, status: "completed" } : item)))
  ...
  return (
    <ExampleCard title="Async Loop" description="Process arrays of items asynchronously" code={code} onRun={handleRun}>
        ...
        {processedItems.length > 0 && (
          ...
          >
            ...
            <div className="text-xs bg-white dark:bg-zinc-800 p-2 rounded border border-zinc-200 dark:border-zinc-700">
              <pre className="overflow-auto">{JSON.stringify(processedItems, null, 2)}</pre>
            </div>
          </motion.div>
        )}
        ...
        {isComplete && (
          ...
          >
            ...
            <div className="mt-1 text-xs text-green-700 dark:text-green-300">
              The async loop pattern allows you to process each item in sequence, waiting for each operation to complete
